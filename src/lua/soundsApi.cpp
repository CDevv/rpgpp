...
void lua_sounds_loadMusic(const std::string &id) { Game::getSounds().loadMusic(id); }

void lua_sounds_playMusic() { Game::getSounds().playMusic(); }

void lua_sounds_playSound(const std::string &id) { Game::getSounds().playSound(id); }

void lua_sounds_set(sol::state_view &lua) {
	auto space = lua["Sounds"].get_or_create<sol::table>();
	space.set_function("PlaySound", &SoundService::playSound, Game::getSounds());
