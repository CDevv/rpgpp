...
const Hotkey HotkeyService::unpack(int packed) {
	return Hotkey{(bool)((packed >> 0) & 1), (bool)((packed >> 1) & 1), (bool)((packed >> 2) & 1),
				  (bool)((packed >> 3) & 1), static_cast<KeyboardKey>(packed >> 4)};
}
...
std::string HotkeyService::registerHotkeyCallback(const std::string &keyId, std::function<void()> cb) {
	std::string uniqueHkCbId = get_uuid();
	hotkeysCb[uniqueHkCbId] = {keyId, cb};
	return uniqueHkCbId;
}

void HotkeyService::unregisterHotkeyCallback(const std::string &uniqueHkCbId) { hotkeysCb.erase(uniqueHkCbId); }

void HotkeyService::addHotkey(const std::string &keyId, const Hotkey &keys) { hotkeyMap[keyId] = keys; }

void HotkeyService::removeHotkey(const std::string &keyId) { hotkeyMap.erase(keyId); }

const HotkeyMap HotkeyService::listHotkeys() { return this->hotkeyMap; }

void HotkeyService::fire() {
	for (auto &[keyId, keys] : hotkeyMap) {
		if ((keys.ctrl ^ IsKeyDown(KEY_LEFT_CONTROL))) continue;
		if ((keys.shift ^ IsKeyDown(KEY_LEFT_SHIFT))) continue;
		if ((keys.alt ^ IsKeyDown(KEY_LEFT_ALT))) continue;
		if ((keys.super ^ IsKeyDown(KEY_LEFT_SUPER))) continue;
