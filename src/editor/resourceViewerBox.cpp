...
void ResourceViewerBox::setRect(Rectangle m_rect) { this->rect = m_rect; }

void ResourceViewerBox::onNewButton() const {
  EditorInterfaceService &ui = Editor::getUi();
  WindowContainer &windows = ui.getWindowContainer();
  auto fileTypeActive = static_cast<EngineFileType>(dropdownActive);
...
void ResourceViewerBox::update() const {
  // not implemented yet, shut the fuck up linter
}
...
void ResourceViewerBox::drawResourcesList() {
  ...
  std::vector<std::string> paths =
      fs.getProject()->getTypePaths(fileTypeActive);
  for (std::string path : paths) {
    if (std::string fileName = GetFileNameWithoutExt(path.c_str());
        ImGui::Button(fileName.c_str(),
                      ImVec2{ImGui::GetWindowWidth(), 24.0f})) {
...
void ResourceViewerBox::draw() {
  constexpr auto resourceButtonSize = ImVec2{76, 26};
  ...
  if (fs.getProject() != nullptr) {
    ...
    if (ImGui::Begin("Project", nullptr,
      ...
      std::string buttonText = TextFormat("%s %s", ICON_KI_CARET_BOTTOM,
                                          arr[dropdownActive].c_str());
...
void ResourceViewerBox::drawContextMenu(const std::string &resPath) {
  ...
  if (ImGui::BeginPopupContextItem()) {
    ...
    std::string fullPath =
        TextFormat("%s/%s", fs.getProject()->getProjectBasePath().c_str(),
                   resPath.c_str());
