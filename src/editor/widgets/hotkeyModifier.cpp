...
HotkeyModifier::HotkeyModifier(const char *typeName, bool initRenderer)
	...
	this->button->onClick([this]() {
		...
		if (modifingState == State::DEFAULT) {
			...
			this->button->setText(ts.getKey("widget.hotkey_modifier.listening"));
		}
	});
	m_container->add(this->button);
}

HotkeyModifier::Ptr HotkeyModifier::create() { return std::make_shared<HotkeyModifier>(); }
...
tgui::Widget::Ptr HotkeyModifier::clone() const { return std::make_shared<HotkeyModifier>(*this); }

void HotkeyModifier::setKey(const std::string &id, KeyboardKey key, bool isShift, bool isCtrl, bool isAlt, bool isSuper,
							bool override) {
	if (modifingState == State::DEFAULT && !override) return;
	keys.clear();
	if (isSuper) {
...
void HotkeyModifier::keyPressed(const tgui::Event::KeyEvent &event) {
	...
	setKey(this->id, tguiToRaylibKey(event.code), event.shift, event.control, event.alt, event.system);
